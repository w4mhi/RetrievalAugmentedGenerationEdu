...
{
    // RagEngine messages
    public const string RagEngineInitialized = "RagEngine initialized with {RetrievalStrategy} retrieval strategy, " +
        "TopK={TopK}, LLM={LlmEnabled}";
    ...
    public const string StartingDocumentIndexing = 
        "Starting document indexing from: {DirectoryPath}";
    ...
    public const string IndexedSuccessfully = 
        "Successfully indexed {ChunkCount} chunks in {ElapsedSeconds}s";
    public const string IndexingFailed = 
    ...
    public const string IndexStatsRetrieved = 
        "Index contains {ChunkCount} chunks, last indexed: {LastIndexed}";
    public const string IndexStatsRetrievalFailed = 
        "Failed to retrieve index statistics: {ErrorMessage}";
    public const string NoSearchResults = "No search results available for LLM generation";
    public const string SendingPromptToLlm = 
        "Sending prompt to LLM with {ContextLength} characters";
    public const string LlmGeneratedAnswer = 
        "LLM generated answer with {AnswerLength} characters";
    ...
    // Resilience messages
    public const string CircuitBreakerOpened = 
        "{OperationName}: Circuit breaker opened for {DurationSeconds}s due to {ExceptionType}";
    public const string CircuitBreakerReset = 
        "{OperationName}: Circuit breaker reset, operations resuming";
    public const string CircuitBreakerHalfOpen = 
        "{OperationName}: Circuit breaker half-open, testing recovery";
    public const string RetryAttempt = 
    ...
    public const string LlmInferenceTimedOut = 
        "{OperationName}: LLM inference timed out after {TimeoutSeconds}s";
    public const string LlmOperationFailed = 
        "{OperationName}: LLM operation failed, using fallback: {ErrorMessage}";
    public const string VectorStoreCircuitBreakerOpened = "{OperationName}: Vector store circuit breaker " +
        "opened for {DurationSeconds}s (possible disk/DB issues)";
    public const string VectorStoreCircuitBreakerReset = 
        "{OperationName}: Vector store circuit breaker reset";
    public const string VectorStoreRetry = "{OperationName}: Vector store retry {RetryCount}/{MaxRetries} " +
        "after {DelayMs}ms: {ErrorMessage}";
    public const string DocumentLoadingFailed = 
        "{OperationName}: Failed to process file {FilePath}, skipping: {ErrorMessage}";
    
    // Vector Store messages
    public const string InitializingVectorStore = 
        "Initializing ChromaVectorStore with persist directory: {PersistDirectory}";
    public const string VectorStoreReady = "ChromaDB collection ready: {CollectionName} at {PersistDirectory}";
    public const string VectorStoreInitializationFailed = 
        "Failed to initialize ChromaVectorStore: {ErrorMessage}";
    ...
    public const string SearchingVectorStoreWithStrategy = 
    ...
    public const string InitializingLanguageModel = "Initializing Phi-4 model from path: {ModelPath}, " +
        "maxTokens: {MaxTokens}, temperature: {Temperature}";
    public const string LanguageModelLoaded = "Successfully loaded Phi-4 model from: {ModelPath}";
    public const string LanguageModelInitializationFailed = 
        "Failed to initialize Phi-4 model from {ModelPath}: {ErrorMessage}";
    public const string GeneratingResponse = "Generating response for prompt of length {PromptLength}, " +
        "maxTokens: {MaxTokens}, temperature: {Temperature}";
    public const string ResponseGenerated = "Successfully generated response of length: {ResponseLength}";
    public const string ResponseGenerationFailed = 
        "Failed to generate response from Phi-4 model: {ErrorMessage}";
}
