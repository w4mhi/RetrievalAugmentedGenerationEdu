...
/// <summary>
/// GPT (OpenAI) language model implementation using Semantic Kernel and OpenAI API.
/// Single Responsibility: Handles GPT model inference only.
/// Dependency Inversion: Implements ILanguageModel abstraction.
/// Supports GPT-4, GPT-4 Turbo, GPT-3.5 Turbo, and other OpenAI models.
/// </summary>
public sealed class GptLanguageModel : ILanguageModel, IDisposable
{
    private readonly Kernel kernel;
    private readonly IChatCompletionService chatService;
    private readonly int defaultMaxTokens;
    private readonly float defaultTemperature;
    private readonly ILogger<GptLanguageModel>? logger;
    ...
    /// <summary>
    /// Initializes the GPT model with Semantic Kernel.
    /// </summary>
    /// <param name="apiKey">OpenAI API key.</param>
    /// <param name="modelName">OpenAI model name (e.g., "gpt-4", "gpt-4-turbo", "gpt-3.5-turbo").</param>
    /// <param name="organizationId">Optional OpenAI organization ID.</param>
        ...
        ILogger<GptLanguageModel>? logger = null)
    {
        ...
        try
        {
            // Build Semantic Kernel with OpenAI connector
        ...
        {
            this.logger?.LogError(ex, "Failed to initialize GPT model {ModelName}: {ErrorMessage}", modelName, ex.Message);
            throw new InvalidOperationException(
                $"Failed to initialize GPT model {modelName}. " +
                $"Ensure API key is valid and OpenAI API is accessible.",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            this.logger?.LogError(ex, "Failed to generate response from GPT model: {ErrorMessage}", ex.Message);
            throw new InvalidOperationException(
                $"Failed to generate response from GPT model: {ex.Message}",
                ex);
        }
    }
